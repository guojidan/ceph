...
tl::expected<std::pair<pg_t, std::string>, std::string>
parse_pg_and_object(const std::vector<std::string>& args, size_t start_idx) {
  ...
  return std::make_pair(*pgid_result, args[start_idx + 1]);
}
  ...
  seastar::logger& logger)
{
  ...
  if (!op.object.has_value() || op.object->empty()) {
    ...
    if (is_json) {
      ...
      // Not JSON, treat as a name and do a lookup.
      ...
      co_return true; // Lookup successful, shard id is set.
    }
  }

  // 3. Find shard for JSON objects or pgmeta
  // This part is only reached if we didn't do a name lookup
  auto pgs = co_await st.list_pgs();
  auto it = std::find_if(pgs.begin(), pgs.end(),
                         [&config](const auto& pg) { return pg.first == config.coll; });
  if (it == pgs.end()) {
    logger.error("PG '{}' not found for object", config.coll);
    co_return false;
  }
...
seastar::future<int> run_tool(StoreTool& st, objectstore_config_t& config) {
  ...
  switch (op.op) {
    ...
    case operation_type_t::LIST_ATTRS: {
      ...
      // Output only the attribute keys, one per line, same as ceph-objectstore-tool
...
int main(int argc, const char* argv[])
{
  ...
  if (!vm.count("type")) {
    config.type = "seastore"; // Default for crimson
  }
